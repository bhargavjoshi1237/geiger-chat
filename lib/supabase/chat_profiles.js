...
export async function listProfiles(organizationId = null) {
  if (isPlaygroundMode()) return pg.profiles();
...
// Fetch a set of profiles by id, NOT org-scoped — used to resolve the names of
// message authors / conversation members who aren't in the org-scoped directory
// (a teammate whose profile now points at a different org, or an external
// contact). Returns [] when there's nothing to fetch.
export async function getProfilesByIds(ids = []) {
  if (isPlaygroundMode()) return pg.profilesByIds(ids);
...
export async function findProfile(query) {
  if (isPlaygroundMode()) return pg.findProfile(query);
...
export async function searchProfiles(query, { limit = 20, organizationId = null } = {}) {
  if (isPlaygroundMode()) return pg.searchProfiles(query);
  const q = (query || "").trim().toLowerCase();
