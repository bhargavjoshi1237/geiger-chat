...
export async function listConversations(meId, kind) {
  ...
  try {
    ...
    // 3) messages for last-preview + unread.
...
export async function createChannel({ id, name, topic, visibility = "public", createdBy, memberIds = [] }) {
  ...
  try {
    ...
    if (!conv) {
      const { data: created, error } = await sb
        .from(CONVERSATIONS)
        .insert({ kind: "dm", dm_key: dmKey, created_by: meId })
        .select("*")
        .single();
      if (error) {
...
// channels like #general show up for everyone).
export async function ensurePublicChannelMembership(meId) {
