...
// Ported from geiger-events' scripts/run-sqls.js. Executes every
// supabase/sqls/*.sql in filename order against the project in STRING_URI.
// Statements are split on top-level `;` (dollar-quoted function bodies are kept
// intact), `create table` / `create index` are made idempotent and skipped when
// they already exist, and everything else (alter/policy/trigger/insert…) re-runs
// safely because the SQL files are written idempotently. Add a feature → drop a
// new .sql in the folder and re-run `npm run db:push`.
...
if (!STRING_URI) {
  ...
  console.error(
    "Add STRING_URI (the direct Postgres connection string for the shared\n" +
      "Supabase project) to .env — you can copy it from a sibling app such as\n" +
      "geiger-events/.env, since the suite shares one project.",
  );
  process.exit(1);
}
...
// This app's own tables — the only ones --clean is allowed to drop. They live in
// the dedicated `chat` schema, so dropping the schema (CASCADE) only affects this
// app and never touches other suite apps. Listed for reference / logging.
const OWN_SCHEMA = "chat";
const OWN_TABLES = [
  "notifications",
  "files",
  "calls",
...
async function run() {
  ...
  try {
    ...
    // NOTE: this Supabase project is SHARED across the Geiger suite, so a blanket
    // "drop all flow_* tables" would destroy other apps' data. --clean here is
    // deliberately scoped to this app's own flow_chat_* tables only.
    if (process.argv.includes("--clean")) {
      console.log(`Dropping the '${OWN_SCHEMA}' schema (chat app only)...`);
      for (const table of OWN_TABLES) {
        await client.query(`DROP TABLE IF EXISTS ${OWN_SCHEMA}.${table} CASCADE`);
        console.log(`  dropped ${OWN_SCHEMA}.${table}`);
      }
